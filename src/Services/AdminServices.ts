...
class AdminService {
    ...
    async searchAdmin(searchText:string,searchField:string): Promise<IAdmin[]> {
        return await Admin.find({[searchField]:{$regex:searchText,$options:'i'} }).exec();
    }

    async getAdminByAdminName(adminName:string): Promise<IAdmin | null> {
        return await Admin.findOne({ adminName: adminName }).exec();
    }
